...
import { useUpdateTicketMutation } from '../../../store/services/supportApi';
...
export const TicketSidebar: React.FC<TicketSidebarProps> = ({ ticket, lineOfBusinessData }) => {
 ...
 const getNameLabel = (p: any): string => {
  ...
  if (typeof p.name === 'object' && p.name) return p.name.roleName || p.name.name || 'Unknown';
 ...
 const handleAddMembers = async () => {
  ...
  const currentIds = currentAssignees.map((a: any) => a._id || a);
 ...
 const availableMembers = (teamMembersData?.teamMembers || [])
  .filter((member: any) => !ticket?.assignedToIds?.some((a: any) => (a._id || a) === member._id));
  ...
  <>
    ...
    <div className="space-y-4">
       ...
       style={{
        backgroundColor:
         ticket?.status === 'New' ? '#3B82F6' : // Blue
          ticket?.status === 'In Progress' ? '#EAB308' : // Yellow
     ...
     <div className="flex justify-between items-center border-b dark:border-gray-700 pb-3">
      ...
      <span className="text-xs font-bold uppercase" style={{ color: lineOfBusinessData?.primaryColor || 'var(--primary)' }}>
       {getRoleLabel(ticket?.escalationLevel)}
      </span>
     </div>
   ...
   <div className="border dark:border-gray-700 p-6 shadow-sm space-y-4" style={{ backgroundColor: 'var(--accent-white)' }}>
    ...
    <div className="space-y-3">
     {(!ticket?.assignedToIds || ticket?.assignedToIds.length === 0) ? (
      ...
      ticket.assignedToIds.map((assignee: any) => (
       <div key={assignee._id} className="flex items-center justify-between group">
        <div className="flex items-center gap-3">
         <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden shadow-sm">
          {assignee.avatar ? (
   ...
   <div className="border dark:border-gray-700 p-6 shadow-sm space-y-4 " style={{ backgroundColor: 'var(--accent-white)' }}>
    ...
    <div className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/50 p-3 border dark:border-gray-700">
     <div className="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-700 shrink-0 overflow-hidden shadow-sm">
      {ticket?.creatorId?.avatar ? (
       <img src={ticket?.creatorId.avatar} className="w-full h-full object-cover" />
      ) : (
       <div
        className="w-full h-full flex items-center justify-center text-[12px] font-bold text-white transition-colors"
        style={{ backgroundColor: lineOfBusinessData?.primaryColor || 'var(--primary)' }}
       >
        {(ticket?.creatorId?.firstName?.[0] || ticket?.creatorId?.name?.[0] || '?').toUpperCase()}
       </div>
      )}
     </div>
    ...
    <div className="p-6 space-y-6">
     <Dropdown
      ...
      options={availableMembers.map((member: any) => ({
       value: member._id,
       label: `${getNameLabel(member)} (${getRoleLabel(member.role)})`
      }))}
