				...
				<div className="flex items-start gap-4 mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-[var(--radius)]">
					...
					<div>
						...
						<p className="text-[12px] text-red-700 dark:text-red-300">
							Deletions will purge all team members, dispositions, campaigns, integrations, settings, and your personal account data. Once started, this process cannot be stopped or undone.
						</p>
					</div>
				</div>
                    ...
                    <div className="flex items-start space-x-3 cursor-pointer group" onClick={() => setAgreedToTerms(!agreedToTerms)}>
                        <div 
                            className={`mt-0.5 w-4 h-4 rounded border flex items-center justify-center transition-all ${
                                agreedToTerms ? 'bg-red-600 border-red-600' : 'border-gray-300 dark:border-gray-600 group-hover:border-red-400'
                            }`}
                        >
                            {agreedToTerms && (
