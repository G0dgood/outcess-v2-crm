...
export const lineOfBusinessApi = createApi({
    ...
    baseQuery: fetchBaseQuery({ 
        ...
        prepareHeaders: (headers, { getState }) => {
            ...
            const token = state.auth?.tokens?.accessToken || localStorage.getItem('token');
            if (token) {
    ...
    endpoints: (builder) => ({
        ...
        updateLineOfBusiness: builder.mutation<unknown, { id: string; data: unknown }>({
            query: ({ id, data }) => ({
        ...
        updateBusinessHours: builder.mutation<unknown, { id: string; data: BusinessHourPayload | BusinessHourPayload[] }>({
            query: ({ id, data }) => ({
        ...
        upsertShiftHour: builder.mutation<unknown, { id: string; data: ShiftHourPayload }>({
            query: ({ id, data }) => ({
