...
import {
  resolveMultiDropdownLevels,
  getAllCampaignDispositions,
} from "@/utils/dispositionMultiDropdown";
...
const ReportPage: React.FC = () => {
  ...
  const userRoleName =
    typeof user?.role === "object"
      ? (user?.role as { roleName?: string })?.roleName
      : user?.role;
  ...
  const isSupervisor =
    user?.isSupervisor === true || userRoleName?.toLowerCase() === "supervisor";
  ...
  const isAgent =
    !isAdmin && !isSuperAdmin && !isSupervisor && !allBucketAccess;
  ...
  const allBuckets = useMemo(() => {
    return (campaignData?.dashboardSettings?.buckets ||
      ...
      []) as unknown as BucketWithMembers[];
  }, [campaignData, setupData]);

  const accessibleBuckets = useMemo(() => {
    return hasFullBucketAccess
      ? allBuckets
  ...
  const [filterType, setFilterType] = useState<
    "today" | "yesterday" | "last7days" | "last30days" | "all" | "dateRange"
  >("today");
      ...
      if (Array.isArray(item.fillDisposition)) {
        item.fillDisposition.forEach((field: DispositionField) => {
          if (
            ...
            const dispDef = configuredDispositions.find(
              (d) => d.name === field.fieldName,
            );
            const levels = resolveMultiDropdownLevels(
              field.fieldName,
  ...
  const fetchAllReportsToExport = async (): Promise<ReportItem[]> => {
    ...
    if (isAgent) {
      ...
      response = (await triggerGetCampaignReport(queryParams).unwrap()) as
        | ReportApiResponse
        | ReportItem[];
    }
  ...
  const totalPages =
    apiData && !Array.isArray(apiData) && typeof apiData.pagination === "object"
  ...
  return (
    <div>
      ...
      {/* Report Table */}
      <div
        className="dark:bg-gray-800 border dark:border-gray-700 overflow-hidden rounded-(--radius)"
        style={{
          backgroundColor: "var(--accent-white)",
          borderColor: "var(--light-gray)",
        }}
          ...
          >
            <thead>
              <tr>
                {dynamicHeaders.length > 0 ? (
                  ...
                  <th className="px-6 py-3 text-left text-[8px] md:text-[10px] font-medium uppercase tracking-wider whitespace-nowrap">
            ...
            >
              {isLoading ? (
                <SVGLoaderFetch
                  colSpan={
                    dynamicHeaders.length > 0 ? dynamicHeaders.length : 1
                  }
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span className="cursor-pointer">
                                {String(report[header] || "-").substring(
                                  0,
                                  tooltipLength,
                                )}
                                ...
