...
const formatValue = (key: string, value: unknown): React.ReactNode => {
  ...
  if (typeof value === 'boolean') {
    return (
      <span
        className={`px-2 inline-flex text-[8px] md:text-[10px] leading-5 font-semibold rounded-full ${value
  ...
  if (key === 'status') {
    return (
      <span
        className={`px-2 inline-flex text-[8px] md:text-[10px] leading-5 font-semibold rounded-full ${value === 'Active'
          ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
          : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
          }`}
      >
