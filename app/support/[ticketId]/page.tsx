...
export default function TicketDetailsPage({ params }: { params: Promise<{ ticketId: string }> }) {
 ...
 }, [initialMessages, socket, ticketId]);
  ...
  <div className="p-0 flex flex-col" style={{ height: 'calc(95vh - 100px)' }}>
   ...
   <div className="flex items-center justify-between gap-4 mb-6 shrink-0">
    ...
    <div className="flex items-center gap-3">
     ...
     {(user?.role === 'supervisor' || user?.role === 'admin') && (typeof ticket?.escalationLevel === 'object' ? (ticket?.escalationLevel as any)?.roleName : ticket?.escalationLevel) !== 'SuperAdmin' && (
      <Button
       variant="outline"
       size="md"
       onClick={() => {
        const currentLevel = typeof ticket?.escalationLevel === 'object' ? (ticket?.escalationLevel as any)?.roleName : ticket?.escalationLevel;
   ...
   <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
    ...
    <div className="w-full lg:w-80 shrink-0 flex flex-col gap-6 overflow-y-auto no-scrollbar min-h-0">
     <TicketSidebar ticket={ticket} lineOfBusinessData={lineOfBusinessData} />
    </div>
